  ...
  import('xlsx').then(XLSX => {
    const templateData = [
      ...
      },
      {
        ...
        'Passage Text (Đoạn văn)':
          'We are proud of our _______ in the tech industry.',
      },
    ];
...
export const CreateTestForm = () => {
  ...
  const [contentQuestionsToUpsert, setContentQuestionsToUpsert] = useState<
    any[] | null
  >(null);
  ...
  const { refetch: fetchP1 } = useFetchQuestions({
    testSetId,
    part: '1',
    limit: 6,
  });
    ...
    createTestSetMutation.mutate(
      ...
      },
      {
        onSuccess: newSet => {
          ...
          setStep(2);
          toast.success('Đã tạo đề thi thành công. Mời bạn tải lên file Đáp án.');
        },
  ...
  const handleAnswerKeyUpload = async (
    ...
    reader.onload = async evt => {
      ...
      upsertQuestionsMutation.mutate(
        ...
        {
          onSuccess: () => {
            toast.success(`Đã tải lên đáp án cho ${questionsToUpsert.length} câu hỏi!`);
            fetchP1().then(res => {
              if (res.data?.data) {
                setPart1Questions(
                  res.data.data.sort(
          ...
          onError: () => toast.error('Lỗi khi lưu đáp án.'),
        },
      );
    };
  ...
  const handleContentUpload = async (
    ...
    reader.onload = async evt => {
      ...
      toast.success(`Đã tải file thành công (${questionsToUpsert.length} câu hỏi). Hãy nhấn "Lưu Toàn Bộ Đề Thi" để tải lên hệ thống.`);
    };
    reader.readAsBinaryString(file);
  };
  ...
  const handleFinalSave = async () => {
    const hasContent =
      contentQuestionsToUpsert && contentQuestionsToUpsert.length > 0;
    ...
    if (!hasContent && !hasAudio && !hasPart1Images) {
      toast.error('Chưa có dữ liệu nào để lưu. Hãy tải lên file Nội dung hoặc file audio.');
      return;
    }

    setIsFinalSaving(true);
    
    toast.info('Đang tải các file lên Cloudinary, vui lòng không tắt trang...');
    ...
    try {
      ...
      if (allQuestions.length > 0) {
        upsertQuestionsMutation.mutate(
          ...
          {
            onSuccess: () => {
              toast.success(`Hoàn tất! Đã lưu đề thi và ${allQuestions.length} câu hỏi.`);
              setStep(5);
        ...
        toast.success('Hoàn tất! Đã lưu audio cho đề thi.');
        setStep(5);
        setIsFinalSaving(false);
      }
    } catch (err) {
      console.error(err);
      toast.error('Đã xảy ra lỗi trong quá trình tải file. Vui lòng thử lại.');
      setIsFinalSaving(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Quy trình 4 bước để khởi tạo và tải lên dữ liệu cho một bộ đề hoàn
              chỉnh.
            </p>
          </div>
          <Button
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Ví dụ: TOEIC Mock Test 2026"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Dùng để thi cuối khóa..."
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
        ...
        {step === 2 && (
          <Card className="shadow-lg border-primary/20">
            <CardHeader className="bg-primary/5 py-4">
              ...
              <CardDescription>
                Upload danh sách đáp án để hệ thống tự động sinh ra 200 câu hỏi
                (trống nội dung).
              </CardDescription>
            </CardHeader>
        ...
        {step === 4 && (
          <Card className="shadow-lg border-primary/20">
            <CardHeader className="bg-primary/5 py-4">
              ...
              <CardDescription>
                Upload file nội dung câu hỏi cho Part 3, 4, 5, 6, 7. Dữ liệu sẽ
                được tự động ghép nối vào các đáp án đã tạo ở Bước 2 thông qua
                "Số thứ tự câu".
              </CardDescription>
            </CardHeader>
          ...
          <Card className="shadow-lg border-emerald-500/20 bg-emerald-500/5">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center space-y-4">
              ...
              <p className="text-muted-foreground max-w-md">
                Bộ đề đã được tải lên hoàn chỉnh với cả đáp án và nội dung. Bạn
                có thể kiểm tra lại trong danh sách đề thi.
              </p>
