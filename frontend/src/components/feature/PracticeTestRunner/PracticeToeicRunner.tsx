...
export const PracticeToeicRunner = ({ id }: { id: string }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const testSetId = id;

  const { useTestSet, useTestQuestions, useSubmitExamMutation } = useToeic();
  const { data: testSet, isLoading: loadingSet } = useTestSet(testSetId);
  const { data: questions, isLoading: loadingQuestions } =
    useTestQuestions(testSetId);
  ...
  const handleCopyQuestion = () => {
    ...
    let groupPassageText = cQ.passageText;
    if (!groupPassageText && cQ.groupId) {
      const gQ = (questions || []).find(
        (q: any) => q.groupId === cQ.groupId && q.passageText,
      );
      if (gQ) groupPassageText = gQ.passageText;
    }
      ...
      const timer = setTimeout(() => {
        ...
        if (audio) {
          audio.play().catch(err => {
            console.warn(
              'Autoplay prevented by browser policy. User must interact first.',
            );
          });
      ...
      },
      {
        onSuccess: data => {
          ...
          localStorage.setItem(
            `toeic-test-answers-${data.resultId}`,
            JSON.stringify(answers),
          );
          // Redirect to results page, passing the newly created TestResult ID
          router.push(
            `/practice/${testSetId}/results?resultId=${data.resultId}`,
          );
        },
      },
    );
  ...
  if (!testSet || !questions || (questions || []).length === 0) {
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center gap-4 bg-background">
        ...
        <Button onClick={() => router.push(ROUTES.PRACTICE)}>
          Quay lại thư viện
        </Button>
      </div>
    );
          ...
          >
            ...
            <DialogContent className="glass-panel">
              <DialogHeader>
                ...
                <DialogDescription>
                  Bạn đã làm {answeredCount} trên tổng số {(questions || []).length} câu
                  hỏi. Bạn có chắc chắn muốn nộp bài để xem điểm số?
                </DialogDescription>
              </DialogHeader>
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    <div className="space-y-4 p-4 rounded-xl bg-secondary/20 border border-border/30 max-h-[380px] overflow-y-auto w-full">
                      ...
                      {(() => {
                        ...
                        if (!audioUrl && currentQuestion.groupId) {
                          const gQ = (questions || []).find(
                            (q: any) =>
                              q.groupId === currentQuestion.groupId &&
                              q.audioUrl,
                          );
                          audioUrl = gQ?.audioUrl || audioUrl;
                        }
                        return (
                      ...
                      {(() => {
                        ...
                        if (!imageUrl && currentQuestion.groupId) {
                          const gQ = (questions || []).find(
                            (q: any) =>
                              q.groupId === currentQuestion.groupId &&
                              q.imageUrl,
                          );
                          imageUrl = gQ?.imageUrl || imageUrl;
                        }
                        return (
                          imageUrl && (
                      ...
                      {(() => {
                        ...
                        if (!passageText && currentQuestion.groupId) {
                          const gQ = (questions || []).find(
                            (q: any) =>
                              q.groupId === currentQuestion.groupId &&
                              q.passageText,
                          );
                          passageText = gQ?.passageText || passageText;
                        }
                        return (
                          passageText && (
              ...
              <CardContent className="space-y-4 overflow-y-auto max-h-[calc(100vh-12rem)] pr-1">
                ...
                {true ? (
                  ...
                  (['1', '2', '3', '4', '5', '6', '7'] as const).map(part => {
                    const partQuestions = (questions || []).map((q: any, index: number) => ({ q, index }))
                    ...
                    const lastNum =
                      partQuestions[partQuestions.length - 1].index + 1;
